...
exports.verifyMobilePayment = asyncErrorHandler(async (req, res, next) => {
    const { transactionId, phoneNumber, amount, method } = req.body;
  
    // In a real app, you would verify with bKash/Nagad API here
    // This is a simulation - in production you would:
    // 1. Call the payment provider's API to verify the transaction
    // 2. Check if amount matches
    // 3. Verify the transaction is successful
    
    // For demo purposes, we'll assume any transaction ID with length > 5 is valid
    const isValidTransaction = transactionId && transactionId.length > 5;
    
    if (!isValidTransaction) {
      return next(new ErrorHandler('Invalid transaction details', 400));
    }
